...
function parseParameter (param, value, schema) {
  ...
  try {
    ...
    let json = null;
    if (e.json) {
      json = _.assign({}, e.json);
      json.errors = _.merge(json.errors, {
        in: param.in,
        message: _.template(
          "Invalid parameter (<%= name %>): Value failed JSON Schema validation"
        )(
          { name: param.name }
        ),
      });
    ...
    throw errorHandler({
      status: 411,
      message: `The "${param.name}" ${param.in} parameter is invalid (${value === undefined ? param.default : value})`,
      ...json,
    });
  }
}
