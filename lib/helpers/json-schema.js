 ...
 * @constructor
 */
function JsonSchema (schema, param) {
  if (!schema) {
    throw errorHandler({ status: 500, message: "Missing JSON schema" });
  }
  if (schema.type !== undefined && dataTypes.indexOf(schema.type) === -1) {
    throw errorHandler({ status: 500, message: "Invalid JSON schema type: " + schema.type });
  }
...
function parseInteger (schema, value, propPath) {
  ...
  if (_.isNaN(parsedValue) || !_.isFinite(parsedValue) || !dataTypePatterns.integer.test(value)) {
    throw errorHandler({
      status: 400,
      message: `"${propPath || value}" is not a properly-formatted whole number`,
    });
  }
  ...
  if (range) {
    if (parsedValue < range.min || parsedValue > range.max) {
      throw errorHandler({
        status: 400,
        message: `"${propPath || parsedValue}" is not a valid ${schema.format}. Must be between ${range.min} and ${range.max}`,
      });
    }
  }
...
function parseNumber (schema, value, propPath) {
  ...
  if (_.isNaN(parsedValue) || !_.isFinite(parsedValue)) {
    throw errorHandler({ status: 400, message: `"${propPath || value}" is not a valid numeric value` });
  }
  ...
  if (range) {
    if (parsedValue < range.min || parsedValue > range.max) {
      throw errorHandler({
        status: 400,
        message: `"${propPath || value}" is not a valid ${schema.format}. Must be between ${range.min} and ${range.max}`,
      });
    }
  }
...
function parseArrayItems (schema, array, propPath) {
  ...
  for (let i = 0; i < array.length; i++) {
    ...
    try {
      ...
      throw errorHandler({
        status: 400,
        message: `Unable to parse ${propPath || "array"} item at index ${i} (${item}).`,
      });
    }
  }
}
...
function parseFile (schema, value, propPath) {
  ...
  if (!_.isObject(parsedValue) || !_.has(parsedValue, "size")) {
    throw errorHandler({
      status: 400,
      message: `${propPath || "File"} is invalid or corrupted`
    });
  }
  ...
  if (schema.minLength) {
    ...
    if (isNaN(minLength)) {
      throw errorHandler({
        status: 500,
        message: `The "minLength" value in the Swagger API is invalid (${schema.minLength})`,
      });
    }
    ...
    if (parsedValue.size < minLength) {
      throw errorHandler({
        status: 400,
        message: `${propPath || "File"} "${parsedValue.originalname}" is only ${parsedValue.size} bytes. The minimum is ${minLength} bytes`,
      });
    }
  }
  ...
  if (schema.maxLength) {
    ...
    if (isNaN(maxLength)) {
      throw errorHandler({
        status: 500,
        message: `The "maxLength" value in the Swagger API is invalid (${schema.maxLength})`,
      });
    }
    ...
    if (parsedValue.size > maxLength) {
      throw errorHandler({
        status: 413,
        message: `${propPath || "File"} "${parsedValue.originalname}" is ${parsedValue.size} bytes. The maximum is ${maxLength} bytes`,
      });
    }
  }
...
function parseDate (schema, value, propPath) {
  ...
  if (_.isDate(value)) {
    ...
    if (!formatPattern.test(value)) {
      throw errorHandler({
        status: 400,
        message: `"${propPath || value}" is not a properly-formatted ${schema.format}`,
      });
    }
    ...
    if (!parsedValue || isNaN(parsedValue.getTime())) {
      throw errorHandler({
        status: 400,
        message: `"${propPath || value}" is an invalid ${schema.format}`,
      });
    }
  }
  ...
  if (schema.minimum) {
    ...
    if (isNaN(minDate.getTime())) {
      throw errorHandler({
        status: 500,
        message: `The "minimum" value in the Swagger API is invalid (${schema.minimum})`,
      });
    }
    ...
    if (parsedValue < minDate) {
      throw errorHandler({
        status: 400,
        message: `${propPath || "Value"} (${parsedValue}) is less than minimum ${minDate}`,
      });
    }
    ...
    if (schema.exclusiveMinimum === true) {
      if (parsedValue.getTime() === minDate.getTime()) {
        throw errorHandler({
          status: 400,
          message: `${propPath || "Value"} (${parsedValue}) is equal to exclusive minimum ${minDate}`,
        });
      }
    }
  }
  ...
  if (schema.maximum) {
    ...
    if (isNaN(maxDate.getTime())) {
      throw errorHandler({
        status: 500,
        message: `The "maximum" value in the Swagger API is invalid (${schema.maximum})`,
      });
    }
    ...
    if (parsedValue > maxDate) {
      throw errorHandler({
        status: 500,
        message: `${propPath || "Value"} (${parsedValue}) is greater than maximum ${maxDate}`,
      });
    }
    ...
    if (schema.exclusiveMaximum === true) {
      if (parsedValue.getTime() === maxDate.getTime()) {
        throw errorHandler({
          status: 400,
          message: `${propPath || "Value"} (${parsedValue}) is equal to exclusive maximum ${maxDate}`,
        });
      }
    }
  }
